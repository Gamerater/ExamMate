...
class _PomodoroScreenState extends State<PomodoroScreen> with TickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    _controller = AnimationController(vsync: this, duration: const Duration(minutes: 25));
    ...
    // --- NEW: AUTO-LINK ARGUMENT CATCHER ---
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final args = ModalRoute.of(context)?.settings.arguments;
      if (args is Task) {
        setState(() {
          _linkedTask = args;
          if (args.subject != null && _availableSubjects.contains(args.subject)) {
            _selectedSubject = args.subject!;
          } else if (args.subject != null) {
            _selectedSubject = args.subject!;
            if (!_availableSubjects.contains(args.subject!)) {
               _availableSubjects.add(args.subject!);
            }
          }
        });
      }
        ...
        if (mounted) {
          setState(() {
            _todaysTasks = allTasks.where((t) {
              ...
              if (!isDone && (tDate.isAtSameMomentAs(todayStart) || tDate.isAfter(todayStart))) {
            ...
            // Re-merge with any passed argument subjects to prevent overwrite drop
            if (_selectedSubject != "General Focus") uniqueSubjects.add(_selectedSubject);
  ...
  Future<void> _updateTaskProgress({required bool markCompleted, required bool incrementSession}) async {
    ...
    if (tasksString != null) {
      ...
      if (index != -1) {
        ...
        await prefs.setString('tasks_data', json.encode(allTasks.map((e) => e.toMap()).toList()));
        _loadTodaysTasks();
      }
    }
  }
  ...
  void _handleTimerComplete() async {
    ...
    if (_isWorkMode) {
      ...
      String? loggedSubject = _selectedSubject == "General Focus" ? null : _selectedSubject;
      await _pomoService.logSession(duration: _workDuration, subject: loggedSubject);
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    ...
                    children: [
                      const Text("Focus Duration", style: TextStyle(fontWeight: FontWeight.w600)),
                      Text("$tempWork min", style: TextStyle(color: defaultWorkColor, fontWeight: FontWeight.bold)),
                    ],
                  ),
                  Slider(
                    ...
                    onChanged: (val) => setModalState(() => tempWork = val.toInt()),
                  ),
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      Text("$tempBreak min", style: TextStyle(color: breakColor, fontWeight: FontWeight.bold)),
                    ],
                  ),
                  Slider(
                    ...
                    onChanged: (val) => setModalState(() => tempBreak = val.toInt()),
                  ),
                  ...
                  // Toggles
                  SwitchListTile(
                    title: const Text("Sound Alerts", style: TextStyle(fontWeight: FontWeight.w500)),
  ...
  Future<void> _confirmSwitch(bool isWork) async {
    if (!_isRunning) { _switchMode(isWork); return; }
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            child: const Text("Stop & Switch", style: TextStyle(color: Colors.white))
          ),
        ],
      ),
    );
    if (!mounted) return;
    if (shouldSwitch == true) { _resetTimer(); _switchMode(isWork); }
  }

  void _switchMode(bool isWork) {
    if (_isWorkMode == isWork) return;
    setState(() { _isWorkMode = isWork; _updateControllerDuration(); });
  }
        ...
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              ...
              const SizedBox(height: 16),
              Text(_isWorkMode ? "Session Complete" : "Break Over", style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
              
              if (_isWorkMode) ...[
                const SizedBox(height: 8),
                Text("+$_workDuration minutes added", style: const TextStyle(color: Colors.green, fontWeight: FontWeight.w600, fontSize: 15)),
              ],
              ...
              if (_isWorkMode && _linkedTask != null) ...[
                ...
                Container(
                  ...
                  child: Column(
                    children: [
                      Text("You were working on:", style: TextStyle(fontSize: 12, color: Colors.blue[800], fontWeight: FontWeight.w500)),
                      const SizedBox(height: 4),
                      Text(_linkedTask!.title, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15), textAlign: TextAlign.center, maxLines: 2),
                      const SizedBox(height: 16),
                      Row(
                        mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                        children: [
                          TextButton(
                            onPressed: () { _stopFeedback(); Navigator.pop(context); _switchMode(!_isWorkMode); }, 
                            child: const Text("Not Done")
                          ),
                          ElevatedButton(
                            style: ElevatedButton.styleFrom(backgroundColor: Colors.blue, elevation: 0), 
                            onPressed: () { 
                              _updateTaskProgress(markCompleted: true, incrementSession: false); 
                              _stopFeedback(); 
                              Navigator.pop(context); 
          ...
          actions: (_isWorkMode && _linkedTask != null) ? null : [
            TextButton(onPressed: () { _stopFeedback(); Navigator.pop(context); _switchMode(!_isWorkMode); }, child: const Text("Switch Mode")),
            TextButton(onPressed: () { _stopFeedback(); Navigator.pop(context); }, child: const Text("Stay Here", style: TextStyle(fontWeight: FontWeight.bold))),
          ],
        );
      },
    );
      ...
      builder: (context) {
        return Container(
          ...
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const Text("Select a Task", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              const SizedBox(height: 4),
              Text("Link a task to track your focus.", style: TextStyle(fontSize: 13, color: Colors.grey.shade500)),
              const SizedBox(height: 16),
              Expanded(
                child: _todaysTasks.isEmpty
                    ? const Center(child: Text("No incomplete tasks for today!", style: TextStyle(color: Colors.grey)))
                    : ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return ListTile(
                            ...
                            subtitle: task.subject != null ? Text(task.subject!, style: TextStyle(fontSize: 12, color: Colors.grey.shade500)) : null,
                            onTap: () {
                              setState(() {
                                _linkedTask = task;
                                // Auto-select subject if task has one
                                if (task.subject != null && _availableSubjects.contains(task.subject)) {
  ...
  Widget _buildModeButton(String label, bool isWork, Color color) {
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(color: isSelected ? color : Colors.transparent, borderRadius: BorderRadius.circular(20)),
        child: Text(label, textAlign: TextAlign.center, style: TextStyle(fontSize: 13, color: isSelected ? Colors.white : Colors.grey[600], fontWeight: FontWeight.bold)),
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text("Pomodoro", style: TextStyle(color: theme.textTheme.bodyLarge?.color, fontWeight: FontWeight.bold)),
        ...
        child: Column(
          children: [
            ...
            Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text("Focus Mode", style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Colors.grey.shade500, letterSpacing: 0.5)),
                  ...
                  child: DropdownButtonHideUnderline(
                    child: DropdownButton<String>(
                      ...
                      items: _availableSubjects.map((String subject) {
                        ...
                        Color sc = isGen ? Colors.grey : SubjectColorHelper.getColor(subject);
                        return DropdownMenuItem<String>(
                          value: subject,
                          child: Row(
                            children: [
                              Icon(isGen ? Icons.adjust : Icons.bookmark, size: 14, color: sc),
                              const SizedBox(width: 8),
                              Text(subject, style: TextStyle(fontWeight: FontWeight.w600, fontSize: 14, color: theme.textTheme.bodyLarge?.color)),
                            ],
                          ),
                        );
                      }).toList(),
                ...
                GestureDetector(
                  ...
                  child: Row(
                    children: [
                      Icon(_linkedTask != null ? Icons.link : Icons.link_off, size: 14, color: Colors.grey.shade500),
                      const SizedBox(width: 6),
                      Expanded(
                        child: Text(
                          _linkedTask != null ? "Linked to: ${_linkedTask!.title}" : "No task linked. Tap to attach.",
            ...
            Container(
              ...
              decoration: BoxDecoration(color: isDark ? Colors.grey[800] : Colors.grey[200], borderRadius: BorderRadius.circular(24)),
              child: Row(
                children: [
                  Expanded(child: _buildModeButton("Work", true, activeWorkColor)),
            ...
            AnimatedContainer(
              ...
              decoration: BoxDecoration(
                ...
                boxShadow: _isRunning ? [
                  BoxShadow(color: currentColor.withOpacity(0.15), blurRadius: 40, spreadRadius: 5)
                ] : [],
              ),
              child: AnimatedBuilder(
                animation: _controller,
                builder: (context, child) {
                  final count = _controller.duration!.inSeconds * _controller.value;
                  return Stack(
                    ...
                    children: [
                      SizedBox(
                        ...
                        child: CircularProgressIndicator(
                          value: _controller.value, strokeWidth: 16,
                          color: currentColor, backgroundColor: currentColor.withOpacity(0.1), strokeCap: StrokeCap.round,
                        ),
                      ),
                      Column(
                        mainAxisSize: MainAxisSize.min,
                        children: [
                          Text(
                            _isWorkMode ? "Focus Session" : "Recovery Time", 
                            style: TextStyle(fontSize: 14, color: currentColor, fontWeight: FontWeight.w600, letterSpacing: 1.0)
                          ),
                          const SizedBox(height: 4),
                          GestureDetector(
                            ...
                            child: Container(
                              ...
                              child: Text(
                                ...
                                style: TextStyle(fontSize: 64, fontWeight: FontWeight.bold, color: theme.textTheme.bodyLarge?.color, fontFeatures: const [FontFeature.tabularFigures()])
                              ),
                            ),
                          ),
                          Text(
            ...
            Row(
              ...
              children: [
                Opacity(
                  opacity: 0.6,
                  child: IconButton(onPressed: _resetTimer, icon: const Icon(Icons.refresh), iconSize: 26, color: Colors.grey),
                ),
                ...
                GestureDetector(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      shape: BoxShape.circle, 
                      boxShadow: [
                        BoxShadow(color: currentColor.withOpacity(0.4), blurRadius: 15, offset: const Offset(0, 5))
                      ]
                    ),
